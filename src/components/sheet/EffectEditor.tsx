...
export default function EffectEditor({ effect, onChange, onRemove, isNested = false }: Props) {
  ...
  const handleAddTarget = () => {
      ...
      if (['dr', 'immunity_damage', 'vulnerability', 'damage_roll', 'damage_increase', 'change_damage'].includes(newType)) newTarget.damageType = 'balistico';
      if (newType === 'immunity_condition') newTarget.condition = 'Agarrado';
      if (['test_attack', 'damage_roll', 'damage_increase', 'change_damage'].includes(newType)) newTarget.weaponFilter = 'all';
  ...
  const handleCategoryChange = (newCat: string) => {
      ...
      const newTargets = currentTargets.map(t => {
          ...
          if (newType === 'damage_roll') nt.weaponFilter = 'all';
          return nt;
      });
      ...
      <div className="space-y-2 pt-2 border-t border-eden-700/50">
        ...
        {effect.targets.map((target, idx) => {
          ...
          <div key={target.id} className="flex flex-wrap gap-2 items-center bg-eden-900/50 p-2 rounded-lg border border-eden-700/50">
            <select value={target.type} onChange={e => { const val = e.target.value as any; const nt: any = { id: target.id, type: val }; if (val === 'test_skill') nt.skill = 'Luta'; if (val === 'attribute' || val === 'test_attribute') nt.attribute = 'PRE'; if (['dr', 'immunity_damage', 'vulnerability', 'damage_roll', 'damage_increase', 'change_damage'].includes(val)) nt.damageType = 'balistico'; if (val === 'immunity_condition') nt.condition = 'Agarrado'; if (['test_attack', 'damage_roll', 'damage_increase', 'change_damage'].includes(val)) nt.weaponFilter = 'all'; if (val === 'ritual_dt') nt.ritualId = 'all'; if (val === 'proficiency') nt.proficiencyType = 'simples'; updateTargetFull(idx, nt); }} className="flex-1 min-w-[140px] bg-eden-950 border border-eden-700 rounded p-1.5 text-xs text-white outline-none font-bold">
              {effect.category === 'change_dice' ? ( <><option value="test_skill">Aumento de Dados da Perícia</option><option value="test_attribute">Aumento de Dados do Atributo</option><option value="test_attack">Aumento de Dados de Ataque</option><option value="damage_increase">Aumento de Dano (Qtd de Dados)</option></> ) : effect.category === 'add_resistance' ? ( <><option value="dr">Resistência a Dano (RD)</option><option value="immunity_damage">Imunidade a Dano</option><option value="vulnerability">Vulnerabilidade a Dano</option><option value="immunity_condition">Imunidade a Condição</option></> ) : effect.category === 'change_damage' ? ( <option value="damage_roll">Substituir Dano da Arma</option> ) : effect.category === 'gain_proficiency' ? ( <option value="proficiency">Nova Proficiência</option> ) : effect.category === 'instant_heal_damage' ? ( <><option value="pv_current">Alterar PV Atual</option><option value="pe_current">Alterar PE Atual</option><option value="san_current">Alterar SAN Atual</option></> ) : ( <><optgroup label="Combate & Status"><option value="pv_max">PV Máximo</option><option value="pe_max">PE Máximo</option><option value="san_max">SAN Máxima</option><option value="pv_temp">PV Temporário</option><option value="pe_temp">PE Temporário</option><option value="san_temp">SAN Temporário</option><option value="defense">Defesa</option><option value="test_attack">Bônus de Ataque (+X)</option><option value="damage_roll">Adicionar Novo Dano na Arma</option></optgroup><optgroup label="Atributos & Perícias"><option value="attribute">Bônus em Atributo Específico</option><option value="test_skill">Bônus em Perícia (+X)</option></optgroup><optgroup label="Mobilidade & Rituais"><option value="displacement">Deslocamento (Metros)</option><option value="load_max">Carga Máxima</option><option value="ritual_dt">DT dos Rituais</option></optgroup></> )}
            </select>
            ...
            {(['test_attack', 'damage_roll', 'damage_increase'].includes(target.type) || effect.category === 'change_damage') && (
                <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto flex-1">
                    ...
                    {['damage_roll', 'damage_increase'].includes(target.type) && <select value={target.damageType || 'balistico'} onChange={e => updateTargetField(idx, 'damageType', e.target.value)} className="bg-eden-950 border border-eden-700 rounded p-1.5 text-xs text-red-300 capitalize">{ALL_DAMAGE_TYPES.map(d => <option key={d} value={d}>{d}</option>)}</select>}
                </div>
            )}
