...
const CLASSES_DATA = [
  { 
    ...
    initialHP: 20, initialPE: 2, initialSAN: 12, 
    pvPerLevel: 4, pePerLevel: 2, sanPerLevel: 3 
  },
  { 
    id: 'especialista', name: 'Especialista', 
    initialHP: 16, initialPE: 3, initialSAN: 16, 
    pvPerLevel: 3, pePerLevel: 3, sanPerLevel: 4 
  },
  { 
    id: 'ocultista', name: 'Ocultista', 
    initialHP: 12, initialPE: 4, initialSAN: 20, 
    pvPerLevel: 2, pePerLevel: 4, sanPerLevel: 5 
  }
] as const;
...
export default function Step4Classes() {
  ...
  const normalizedOriginSkills: string[] = originSkills.map((s: string) => s.startsWith('Profissão') ? 'Profissão' : s);
  ...
  const calculateRequirements = () => {
      ...
      if (originSkillCount < 2) {
          freeSlots += (2 - originSkillCount);
      }
  ...
  const renameSkill = (skillKey: string) => {
      ...
      const defaultName = (normalizedOriginSkills.includes(skillKey) && skillKey === 'Profissão') ? originName : '';
  ...
  useEffect(() => {
      ...
      if (hasChanges) {
          updateCharacter(prev => ({ ...prev, skills: newSkills }));
      }
