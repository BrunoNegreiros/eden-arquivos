...
import { 
  ...
  ShoppingBag, Plus, Calculator, Edit2, Bomb, Search, Sparkles
} from 'lucide-react';
...
function FormulaBuilder({ formula, onChange }: { formula: Formula, onChange: (f: Formula) => void }) {
    ...
    return (
      <div className="bg-eden-950/50 p-4 rounded-xl border border-eden-700 space-y-3">
          <div className="flex justify-between items-center"><label className="text-xs font-bold text-energia flex gap-2"><Calculator size={14}/> Fórmula</label><button type="button" onClick={addTerm} className="text-xs bg-eden-800 px-2 py-1 rounded hover:bg-eden-700"><Plus size={12}/> Add</button></div>
          {formula.terms.map((t, i) => (
              <div key={t.id} className="flex gap-2 items-center bg-eden-900/30 p-2 rounded border border-eden-700/50 flex-wrap">
                  ...
                  <button type="button" onClick={()=>removeTerm(i)} className="ml-auto text-red-400 p-1 hover:bg-red-500/10 rounded transition-colors"><Trash2 size={14}/></button>
              </div>
          ))}
      </div>
    )
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="bg-eden-900 w-full max-w-5xl max-h-[95vh] rounded-2xl border border-eden-600 shadow-2xl flex flex-col">
        <div className="p-4 border-b border-eden-700 bg-eden-800 rounded-t-2xl flex justify-between items-center shrink-0">
          <h3 className="font-bold text-xl text-eden-100">{initialData ? 'Editar Item' : 'Criar Novo Item'}</h3>
          <button type="button" onClick={onCancel}><X size={24} className="text-eden-100/50 hover:text-white"/></button>
        </div>
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-1 space-y-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {ITEM_TYPES.map(t => (
                            <button key={t.id} type="button" onClick={() => handleChange('type', t.id)} className={`flex flex-col items-center justify-center p-3 rounded-xl border h-20 transition-all ${formData.type === t.id ? `bg-eden-800 ${t.color} shadow-md scale-105` : 'bg-eden-950 border-eden-800 text-eden-100/40 hover:bg-eden-900'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-4 bg-eden-950/30 p-4 rounded-xl border border-eden-700/50">
                        <div className="space-y-1">
                            ...
                            <select value={formData.category} onChange={(e) => handleChange('category', Number(e.target.value))} className="w-full bg-eden-900 border border-eden-700 rounded-lg p-2.5 text-sm text-white">
                    ...
                    {(formData.type === 'weapon' || formData.type === 'explosive') && (
                        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 space-y-4 animate-in slide-in-from-top-2">
                            ...
                            {formData.type === 'weapon' && (
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    <div className="space-y-1"><label className="text-[10px] text-red-200/60 uppercase font-bold">Subtipo</label><select value={formData.subtype || 'melee'} onChange={(e) => handleChange('subtype', e.target.value)} className="w-full bg-eden-900 border border-red-900/50 rounded p-2 text-xs text-white"><option value="melee">Corpo a Corpo</option><option value="ranged">Distância</option></select></div>
                                    <div className="space-y-1"><label className="text-[10px] text-red-200/60 uppercase font-bold">Empunhadura</label><select value={formData.hands || 'one'} onChange={(e) => handleChange('hands', e.target.value)} className="w-full bg-eden-900 border border-red-900/50 rounded p-2 text-xs text-white"><option value="light">Leve</option><option value="one">Uma Mão</option><option value="two">Duas Mãos</option></select></div>
                                    <div className="space-y-1"><label className="text-[10px] text-red-200/60 uppercase font-bold">Complexidade</label><select value={formData.complexity || 'simple'} onChange={(e) => handleChange('complexity', e.target.value)} className="w-full bg-eden-900 border border-red-900/50 rounded p-2 text-xs text-white"><option value="simple">Simples</option><option value="tactical">Tática</option><option value="heavy">Pesada</option></select></div>
                                    <div className="space-y-1"><label className="text-[10px] text-red-200/60 uppercase font-bold">Alcance</label><select value={formData.range || 'curto'} onChange={(e) => handleChange('range', e.target.value)} className="w-full bg-eden-900 border border-red-900/50 rounded p-2 text-xs text-white"><option value="adjacente">Adjacente</option><option value="curto">Curto</option><option value="medio">Médio</option><option value="longo">Longo</option><option value="extremo">Extremo</option></select></div>
                                </div>
                            )}
                            {formData.type === 'weapon' && (
                                <div className="space-y-3 pt-3 border-t border-red-500/20">
                                    <div className="flex justify-between items-center">
                                        ...
                                        {(formData.attackTest?.skill === 'Luta' || (!formData.attackTest?.skill && formData.type === 'weapon')) && (
                                            <label className="flex items-center gap-1.5 cursor-pointer bg-red-900/30 px-2 py-1 rounded text-[10px] text-red-200 hover:bg-red-900/50 transition-colors border border-red-500/20 animate-in fade-in zoom-in-95">
                                                <input 
                                                    type="checkbox" 
                                                    checked={formData.isAgile || false} 
                                                    onChange={e => handleChange('isAgile', e.target.checked)} 
                                                    className="accent-red-500 w-3 h-3 rounded bg-eden-900"
                                                />
                                                Arma Ágil
                                            </label>
                                        )}
                                    </div>
                                    <div className="space-y-1"><label className="text-[10px] text-red-200/60 uppercase font-bold">Perícia Base</label><select value={formData.attackTest?.skill || 'Luta'} onChange={e => updateNested('attackTest', 'skill', e.target.value)} className="w-full bg-eden-900 border border-red-900/50 rounded p-2 text-sm text-white">{SKILL_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                                    <FormulaBuilder formula={formData.attackTest?.secondaryDice || { terms: [], operations: [] }} onChange={f => updateNested('attackTest', 'secondaryDice', f)} />
                                    <FormulaBuilder formula={formData.attackTest?.secondaryBonus || { terms: [], operations: [] }} onChange={f => updateNested('attackTest', 'secondaryBonus', f)} />
                                </div>
                            )}
                            {formData.type === 'explosive' && (
                                <div className="grid grid-cols-2 gap-3 pt-2">
                                    <div className="space-y-1"><label className="text-[10px] font-bold text-red-300">Área</label><input type="text" value={formData.area || ''} onChange={e => handleChange('area', e.target.value)} className="w-full bg-eden-900 border border-red-900/50 rounded p-2 text-xs text-white" placeholder="6m raio"/></div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-red-300">DT Resistência</label>
                                        <div className="flex gap-2">
                                            <select value={formData.dtAttribute || 'AGI'} onChange={e => handleChange('dtAttribute', e.target.value)} className="bg-eden-900 border border-red-900/50 rounded p-2 text-xs text-white outline-none w-20"><option value="none">Fixa</option><option value="AGI">AGI</option><option value="FOR">FOR</option><option value="INT">INT</option><option value="PRE">PRE</option><option value="VIG">VIG</option></select>
                                            <input type="number" value={formData.dt || 0} onChange={e => handleChange('dt', Number(e.target.value))} className="w-full bg-eden-900 border border-red-900/50 rounded p-2 text-xs text-white" placeholder="Bônus"/>
                                        </div>
                                    </div>
                                </div>
                            )}
                    ...
                    <div className="bg-red-500/5 border border-red-500/20 rounded-xl p-4 space-y-4 animate-in slide-in-from-top-2 mt-6">
                        <div className="flex justify-between items-center">
                            <h4 className="text-sm font-bold text-red-300 uppercase flex items-center gap-2"><Crosshair size={16}/> Dano Causado</h4>
                            <button type="button" onClick={() => { const currentDamage = Array.isArray(formData.damage) ? formData.damage : []; handleChange('damage', [...currentDamage, { id: generateId(), diceCount: 1, diceFace: 6, type: 'impacto', bonus: { terms: [], operations: [] }, isMultipliable: true }]); }} className="text-[10px] bg-red-900/50 px-2 py-1 rounded text-red-200 hover:bg-red-800 flex items-center gap-1 transition-colors"><Plus size={12}/> Add Dano</button>
                        </div>
                        {(Array.isArray(formData.damage) ? formData.damage : []).map((dmg: any, idx: number) => (
                            <div key={dmg.id} className="bg-black/40 p-3 rounded-lg border border-red-900/30 space-y-3 relative group">
                                <button type="button" onClick={() => { const newDmg = formData.damage?.filter((_: any, i: number) => i !== idx); handleChange('damage', newDmg); }} className="absolute -top-2 -right-2 bg-red-900 hover:bg-red-700 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><X size={14} /></button>
                                <div className="grid grid-cols-2 gap-3 flex-1">
                                    <div className="space-y-1"><label className="text-[10px] text-red-200/60 uppercase font-bold">Dados</label><div className="flex gap-1 items-center bg-eden-900 border border-red-900/50 rounded p-1"><input type="number" value={dmg.diceCount ?? 0} onChange={e => { const newDmg = [...formData.damage]; newDmg[idx].diceCount = Number(e.target.value); handleChange('damage', newDmg); }} className="w-10 bg-eden-800 text-center text-sm text-white font-bold outline-none" /><span className="text-xs text-red-400 font-bold">d</span><select value={dmg.diceFace || 6} onChange={e => { const newDmg = [...formData.damage]; newDmg[idx].diceFace = Number(e.target.value) as any; handleChange('damage', newDmg); }} className="bg-eden-800 text-sm text-white font-bold outline-none flex-1">{[2,3,4,6,8,10,12,20,100].map(d => <option key={d} value={d}>{d}</option>)}</select></div></div>
                                    <div className="space-y-1"><label className="text-[10px] text-red-200/60 uppercase font-bold">Tipo</label><select value={dmg.type || 'balistico'} onChange={e => { const newDmg = [...formData.damage]; newDmg[idx].type = e.target.value; handleChange('damage', newDmg); }} className="w-full bg-eden-900 border border-red-900/50 rounded p-2 text-sm text-white capitalize">{DAMAGE_TYPES_INFO.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
                                </div>
                                <div className="flex items-center gap-2 mt-1"><input type="checkbox" id={`mult_${dmg.id}`} checked={dmg.isMultipliable !== false} onChange={e => { const newDmg = [...formData.damage]; newDmg[idx].isMultipliable = e.target.checked; handleChange('damage', newDmg); }} className="w-3 h-3 rounded border-red-500 bg-eden-900 text-red-500"/><label htmlFor={`mult_${dmg.id}`} className="text-[10px] font-bold text-red-200/60 uppercase cursor-pointer select-none">Multiplica no Crítico?</label></div>
                        ...
                        <div className="space-y-1 pt-3 border-t border-red-500/20"><label className="text-[10px] text-red-200/60 uppercase font-bold">Crítico (Margem / Multiplicador)</label><div className="flex gap-2"><input type="number" value={formData.critical?.range || 20} onChange={e => updateNested('critical', 'range', Number(e.target.value))} className="w-full bg-eden-900 border border-red-900/50 rounded p-2 text-xs text-white text-center"/><span className="text-white font-bold self-center">/</span><input type="number" value={formData.critical?.multiplier || 2} onChange={e => updateNested('critical', 'multiplier', Number(e.target.value))} className="w-full bg-eden-900 border border-red-900/50 rounded p-2 text-xs text-white text-center"/></div></div>
                    </div>
                    ...
                    {formData.type === 'protection' && (
                        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4 space-y-4 animate-in slide-in-from-top-2">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1"><label className="text-[10px] text-emerald-200/60 uppercase font-bold">Defesa (+)</label><input type="number" value={formData.defenseBonus || 0} onChange={e => handleChange('defenseBonus', Number(e.target.value))} className="w-full bg-eden-900 border border-emerald-900/50 rounded p-2 text-sm text-white font-bold"/></div>
                                <div className="space-y-1"><label className="text-[10px] text-emerald-200/60 uppercase font-bold">Penalidade (-)</label><input type="number" value={formData.penalty || 0} onChange={e => handleChange('penalty', Number(e.target.value))} className="w-full bg-eden-900 border border-emerald-900/50 rounded p-2 text-sm text-white font-bold"/></div>
                            </div>
                            <div className="flex items-center gap-2 pt-2 border-t border-emerald-500/20"><input type="checkbox" id="isHeavy" checked={formData.isHeavy || false} onChange={e => handleChange('isHeavy', e.target.checked)} className="w-4 h-4 rounded border-emerald-500 bg-eden-900"/><label htmlFor="isHeavy" className="text-xs text-emerald-100 font-bold">Proteção Pesada?</label></div>
                        </div>
                    )}
                    ...
                    {formData.type === 'ammo' && (
                        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 space-y-4 animate-in slide-in-from-top-2">
                            ...
                            <div className="space-y-1"><label className="text-[10px] text-yellow-200/60 uppercase font-bold">Tipo de Duração</label><select value={formData.ammoDurationType || 'scenes'} onChange={e => handleChange('ammoDurationType', e.target.value)} className="w-full bg-eden-900 border border-yellow-900/50 rounded p-2 text-xs text-white"><option value="scenes">Por Cenas</option><option value="single_use">Uso Único</option><option value="infinite">Infinita</option></select></div>
                            {(!formData.ammoDurationType || formData.ammoDurationType === 'scenes') && (
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="space-y-1"><label className="text-[10px] text-yellow-200/60 uppercase font-bold">Cenas</label><input type="number" value={formData.durationScenes || 0} onChange={e => handleChange('durationScenes', Number(e.target.value))} className="w-full bg-eden-900 border border-yellow-900/50 rounded p-2 text-sm text-white font-bold"/></div>
                                    <div className="space-y-1"><label className="text-[10px] text-yellow-200/60 uppercase font-bold">Sobras</label><input type="number" value={formData.leftovers || 0} onChange={e => handleChange('leftovers', Number(e.target.value))} className="w-full bg-eden-900 border border-yellow-900/50 rounded p-2 text-sm text-white font-bold"/></div>
                                </div>
                            )}
                        </div>
                    )}

                    <div className="space-y-1 h-full flex flex-col pt-4">
                        <label className="text-xs font-bold text-eden-100/50 uppercase">Descrição & Regras</label>
                        <textarea value={formData.description} onChange={e => handleChange('description', e.target.value)} className="w-full bg-eden-900 border border-eden-700 rounded-lg p-3 text-sm text-eden-100 focus:border-energia outline-none resize-none flex-1 min-h-[100px]" placeholder="Detalhes..."/>
                    </div>
                </div>
                
                <div className="w-full lg:w-[350px] space-y-6 border-t lg:border-t-0 lg:border-l border-eden-700 pt-6 lg:pt-0 lg:pl-6">
                    <div className="bg-black/20 rounded-xl border border-eden-700/50 p-4 space-y-3 h-full flex flex-col">
                        <div className="flex justify-between items-center border-b border-eden-700 pb-2">
                            <h3 className="text-xs md:text-sm font-black text-eden-100 uppercase tracking-widest flex items-center gap-2"><Sparkles size={16} className="text-energia" /> Efeitos</h3>
                            <button type="button" onClick={() => { const newEffect = { id: Date.now().toString(), name: 'Novo Efeito', category: 'add_fixed', value: { terms: [{ id: '1', type: 'fixed', value: 1 }], operations: [] }, targets: [] }; const newEffectsList = [...(formData.effects || []), newEffect]; setFormData((prev: any) => ({ ...prev, effects: newEffectsList })); setEditingEffectIndex(newEffectsList.length - 1); }} className="text-[10px] font-bold text-energia hover:text-yellow-400 flex items-center gap-1"><Plus size={14} /> Novo</button>
                        </div>
                        <div className="space-y-2 flex-1 overflow-y-auto custom-scrollbar pr-1 max-h-[400px]">
                            {(formData.effects || []).map((effect: any, idx: number) => (
                                <div key={effect.id} className="flex justify-between items-center bg-eden-950/50 border border-eden-700/50 p-2.5 rounded-lg group">
                                    <div className="flex flex-col max-w-[200px]"><span className="text-xs font-bold text-white capitalize truncate">{effect.name ? effect.name : effect.category.replace('_', ' ')}</span><span className="text-[10px] text-eden-100/50">{effect.targets?.length || 0} alvo(s)</span></div>
                                    <div className="flex items-center gap-1">
                                        <button type="button" onClick={() => setEditingEffectIndex(idx)} className="p-1.5 text-eden-100/50 hover:text-energia rounded transition-colors"><Edit2 size={16}/></button>
                                        <button type="button" onClick={() => { const newEffects = [...(formData.effects || [])]; newEffects.splice(idx, 1); setFormData((prev: any) => ({ ...prev, effects: newEffects })); }} className="p-1.5 text-eden-100/50 hover:text-red-400 rounded transition-colors"><Trash2 size={16}/></button>
                                    </div>
                                </div>
                            ))}
                            {(!formData.effects || formData.effects.length === 0) && <p className="text-[10px] text-eden-100/30 italic text-center py-4 border border-dashed border-eden-800 rounded-lg">Sem bônus.</p>}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="p-4 border-t border-eden-700 bg-eden-800 rounded-b-2xl flex justify-end gap-3 shrink-0">
            <button type="button" onClick={onCancel} className="px-6 py-2.5 text-eden-100 hover:bg-eden-700 rounded-lg text-sm font-bold">Cancelar</button>
            <button type="button" onClick={() => onSave(formData)} disabled={!formData.name} className="px-8 py-2.5 bg-energia text-eden-900 rounded-lg text-sm font-black hover:bg-yellow-400 shadow-lg disabled:opacity-50">SALVAR ITEM</button>
        </div>
            ...
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[150] flex items-center justify-center p-4">
                <div className="bg-eden-900 border border-eden-600 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
                    <div className="p-4 border-b border-eden-700 bg-eden-800 flex justify-between items-center shrink-0">
                        ...
                        <button type="button" onClick={() => setEditingEffectIndex(null)} className="text-eden-100/50 hover:text-white"><X size={20}/></button>
                    </div>
                    <div className="p-4 overflow-y-auto custom-scrollbar flex-1">
                        <EffectEditor effect={formData.effects[editingEffectIndex]} onChange={(updatedEffect: any) => { const newEffects = [...(formData.effects || [])]; newEffects[editingEffectIndex] = updatedEffect; setFormData((prev: any) => ({ ...prev, effects: newEffects })); }} onRemove={() => { const newEffects = [...(formData.effects || [])]; newEffects.splice(editingEffectIndex, 1); setFormData((prev: any) => ({ ...prev, effects: newEffects })); setEditingEffectIndex(null); }} />
                    </div>
                    <div className="p-4 border-t border-eden-700 bg-eden-800 shrink-0 flex justify-end">
                        <button type="button" onClick={() => setEditingEffectIndex(null)} className="bg-energia text-eden-900 font-black px-6 py-2 rounded-lg hover:bg-yellow-400 shadow-lg">Concluir</button>
                    </div>
                </div>
            </div>
        )}
...
export default function Step6Inventory() {
  ...
  const handleSaveItem = (item: any) => {
      const finalItem = { 
          ...
          modifications: item.modifications || [], // Garantir que arrays existam
          curses: item.curses || []
      };
